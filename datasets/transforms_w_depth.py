...
# custom w,h fixed same size resizing
class Resize():
    def __init__(self, size):
        self.size = size

    def __call__(self, img, depth, target):
        return resize(img, depth, target, self.size)
...
def crop(image, depth, target, region):
    ...
    if "boxes" in target:
        ...
        if "boxes" in target:
            ...
            bool_tensor = cropped_boxes[:, 1, :] > cropped_boxes[:, 0, :]
...
class Normalize(object):
    ...
    def __call__(self, image, depth, target=None):
        ...
        # simple gray scale mapping for 3-channel depth
